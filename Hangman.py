...
def main():
    ...
    root.title("Hangman")
    words = ["RAJKOT", "AHMEDABAD", "DELHI", "MUMBAI", "JAIPUR", "SURAT", "VADODARA", "CHENNAI", "BENGALURU", "KOLKATA",
             "HYDERABAD", "INDORE", "LUCKNOW", "NAGPUR", "UDAIPUR", "AMRITSAR"]
    ...
    Label(root, text="Hangman", fg="red", bg="black", font=("Berlin Sans FB", 32)).grid(row=0, column=1, columnspan=9,
                                                                                        pady=10)
    img = Label(root)
    ...
    lblWord = StringVar()
    Label(root, textvariable=lblWord, font=("Arial 24 bold"), bg='black', fg='red').grid(row=1, column=4, columnspan=6,
                                                                                         padx=10)
    v = StringVar()
    def msg():
        v.set("It's a city")
    ...
    for c in ascii_uppercase:
        Button(root, text=c, command=lambda c=c: guess(c), font=("Arial 18"), width=4, bg='black', fg='white').grid(
            row=r, column=col)
        col += 1
        if count == 9:
            r = 3
    ...
    Button(root, text="New\nGame", command=lambda: newGame(), font=("TkDefaultFont", 12), bg='red', fg='white').grid(
        row=7, column=5, pady=20)
    newGame()
    root.mainloop()
